...
const HackerNews = () => {
  ...
  return (
    <div className="bg-gray-300 mx-auto mt-5 p-5 rounded-lg shadow-md w-2/4">
      <input
        type="text"
        className="bg-gray-100
         mb-5 border border-blue-900
          text-black p-5 mb-5 block w-full
    ...
    border-r-4 border-r-transparent animate-spin mx-auto"
      ...
      <div class="fle flex-wrap gap-5">

      {!loading &&
        hits.length > 0 &&
        hits.map((item, index) => <h3 className="
        p-3 bg-gray-100 rounded-sm
        " key={item.title}>{item.title}</h3>)}
