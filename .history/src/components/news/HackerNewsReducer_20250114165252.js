...
const intialState = {
  ...
  loading: true,
  error: "",
  url: "http://n.algolia.com/api/v1/search?query=''",
};
  ...
  return (
    <div className=" mx-auto mb-5 mt-5 p-5 rounded-lg shadow-md w-2/4">
      <div className="flex mb-5 mt-5 gap-x-5">
        ...
        />
        <button className="bg-blue-500 text-white font-semibold mt-5 p-1 flex-shrink-0 rounded-lg">
          Fetching
        </button>
      </div>
    ...
    border-r-4 border-r-transparent animate-spin mx-auto"
      ...
      {!state && state.error && (
        <p className="text-red-400 my-5">{state.error}</p>
      )}
